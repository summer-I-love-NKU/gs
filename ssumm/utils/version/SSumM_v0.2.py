...
# v0.2 加在筛选相似节点的地方，不影响saving1，
# 我用embedding筛选的2-hop节点一定比随机的要好吧，
# 我不参与你的saving计算，
# 我只是提供了不一样的相似节点对（我认为应该比随机|C|个节点对要好吧）
...
class SSumM:
    def __init__(self, args):
        ...
        self.removed_node = []
        self.useNE = args.useNE#note NEO_0.0
        ...
        self.dataset_name = args.dataPath.split('/')[-1].split('\\')[-1][:-4]  # xxx.txt
    ...
    def computeCost(self,length,tmp,superedgeCost):
        ...
        if self.useNE:#note NEO_1.1
            ...
            # largest=heapq.nlargest(length,similarity.reshape(1,-1)[0])# attn 因为会去重，所以这里可以多选一点
    ...
    def inputGraph(self):
        ...
        if self.useNE:
            if 0:
                ...
                self.NodeEmbedding = getNodeEmb(self.dataPath,self.dataset_name)
    ...
    # node embedding operation : note NEO_1.0 计算
...
def parse_args():
    ...
    general.add_argument('--dataPath', type=str, default='', help='input file')
...
def main():
    ...
    print(f"L{args.re}error:   {REDSTR[0]}{graph.norm()[1]:.4e}{REDSTR[1]}    useNE: {args.useNE}")  # flag main
