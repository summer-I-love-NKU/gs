...
# v0.1 感觉设计有问题，
# saving2属于是帮倒忙，
# 本来saving1和目标函数（RE2）紧密关联，加上saving2显然让结果不会更好只会更差，起到了扰乱作用
...
class SSumM:
    def __init__(self, _dataPath: str, _outputfolder: str, _fracK: float, _random_seed: int, _use_fast_topndrop: int,urd:int,useNE:int):
    ...
    def mergeStep(self, similar_node_set, threshold):
        ...
        for _i,tmp in enumerate(similar_node_set):
            ...
            while numskip_patience < max(math.log2(length), 1):#_attn 2024.1.4发现第2个bug，这里也要改成length，必须用length而不是len(tmp)！因为tmp不会真的删除元素，只是把length缩减了！！！#while numskip_patience < max(math.log2(len(tmp)), 1):
                ...
                for i in range(length):
                    ...
                    saving1 = (1 -self.alpha) * self.savingMDL(tmp[src], tmp[dst], superedgeCost)
                    saving2= self.alpha* self.savingEmbeddingSim(tmp[src], tmp[dst])#flag savingMDL note NEO_02
                ...
                # Update variables
                superedgeCost = 2 * math.log2(self.numSuperNodes) + math.log2(self.maxWeight)
                summarySize = self.numNodes * math.log2(self.numSuperNodes) + self.numSuperEdges * (2 * math.log2(self.numSuperNodes) + math.log2(self.maxWeight))
    ...
    def inputGraph(self):
        ...
        # note NEO_04
        if self.useNE:
            from getNE import getNodeEmb
            self.NodeEmbedding=getNodeEmb(self.dataPath)
            print("[ get NodeEmbedding OK! ]")
    ...
    def dataCostPair(self, src, dst):
        ...
        if self.superGraph[src].get(dst,0) > 0:#if tmpEdgeNum > 0:#_attn 2024.1.4发现第3个bug：这里应该是 if(superGraph[src].get(dst,0) > 0)判断是否是sparse情况,因为这个值可能为负数！ 不能用tmpEdgeNum因为tmpEdgeNum一定是>=0的
    ...
    def TotalCostPair_merge(self, src, dst, n):
        ...
        # Out[135]: 2
        if n == src:
            tmpEdgeNum = ((source.get(src,0) & 0x7FFF_FFFF) + 2 * (source.get(dst,0) & 0x7FFF_FFFF) + (dest.get(dst,0) & 0x7FFF_FFFF)) // 2
    ...
    # node embedding operation : note NEO_01
    def savingEmbeddingSim(self, src, dst):#flag savingEmbeddingSim
        if not self.useNE:
            return 0
    ...
    def merge(self, src, dst):
        ...
        for n in merged_NodeSet:
            denseNum = 0
            if n == src:
                tmpEdgeNum = (source.get(src,0) & 0x7FFF_FFFF) + 2 * (source.get(dst,0) & 0x7FFF_FFFF) + (dest.get(dst,0) & 0x7FFF_FFFF)
                if source.get(src,0) > 0:
                    denseNum += 1
                if source.get(dst,0) > 0:
                    denseNum += 1
                if dest.get(dst,0) > 0:#_attn 2023.1.4 第4个bug 看到删除159个节点后，numSuperEdges不一样！导致summarySize不一样，而涉及到它的写入，一处是numSuperEdges -= (denseNum - 1);另一处是dropEdgesAndmergeIsolated那里，猜测 大概率与 denseNum有关，终于发现这里的粗心，是dest.get(dst,0)而不是dest.get(src,0)！！！
                    denseNum += 1
            else:
                tmpEdgeNum = (source.get(n,0) & 0x7FFF_FFFF) + (dest.get(n,0) & 0x7FFF_FFFF)
...
def parse_args():#flag args
    ...
    general.add_argument('--useNE', type=int, default=1,
